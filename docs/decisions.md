...
- One ApplicationSet for every service, and onboarding one is an entry in its list: a repository, a branch, and the stage that branch deploys. Discovering the repositories instead, through GitHub's API, would need a token in the cluster and would still have to read each service's values for its name and team. What stays is the blast radius: one unreadable values file stops the generator for every service.
