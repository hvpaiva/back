...
- The cache's password is generated by its own Composition. Nothing else in the cluster generates one, so the Composition reads back the Secret it wrote and only mints a password when there is none. Deleting that Secret means a new password and a restarted server.
