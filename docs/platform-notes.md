...
### A sync in the middle of a canary rewrites the route's weights

Argo Rollouts moves a canary's share of the requests by writing weights into the service's HTTPRoute. The services' Applications leave the weights out of the comparison (`ignoreDifferences`), so an Application stays *Synced* while they change, but a sync still applies the route as the chart renders it. On a canary taking a fifth of the requests, a sync that changed nothing turned the weights 80 and 20 into 1 and 1, the default, and the canary took half the requests for the eleven seconds until its next step: Argo Rollouts didn't write them back in between. `RespectIgnoreDifferences=true`, the option meant to apply the live value of an ignored field, didn't keep a weight either, whether the sync came from Git or from a local folder, and whether the field was named by a jq path or a JSON pointer.

So the chart renders the weights as 100 and 0, and a rewrite sends every request back to the stable version rather than half of them to the canary. The Applications also sync with `ApplyOutOfSyncOnly=true`, which applies only what changed: a sync that added an object in the middle of a canary left 80 and 20 in place, and the same sync without that option turned them into 100 and 0.

### A deploy that also removes something waits for its canary

With `PruneLast=true`, a sync deletes what it replaces only after everything it applied is healthy, and a Rollout in the middle of a canary isn't: Argo CD reads it as *Suspended* while a step waits. A change that ships a new version and removes an object in the same sync holds that sync for the whole canary. It took sixty seconds when measured, a second sync of the same Application was refused meanwhile (`another operation is already in progress`), and the removed object went only when the canary finished. A change that only ships a version doesn't wait: its sync ends once the Rollout is applied.

### Going back from a Rollout to a Deployment drops requests
